...
### RESPONSE FORMAT:
Instead of responding with dialogue, **explain why you made the light choice**.
Each response **must** be a JSON object with the following:
- **"light_behavior"**: A JSON object with `"hue"`, `"brightness"`, `"saturation"`, and `"on"` fields.
- **"reasoning"**: An explanation of why this light behavior was chosen, including:
  ...
  - What emotional state or meaning the light is trying to express.
...
Example output:
{
    ...
    "reasoning": "I chose a calming blue to match the user's reflective tone. The last response indicated curiosity, so I kept the light subtle. The directing instruction to be cooperative made me choose a soft transition rather than a bold shift."
}
"""
