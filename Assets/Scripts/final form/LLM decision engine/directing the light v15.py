...
def analyze_speech(text):
    ...
    try:
        ...
        # AI decides when to use pulse effect
        if "happy" in mood_json["reasoning"].lower() or "excited" in mood_json["reasoning"].lower():
            mood_json["light_behavior"]["effect"] = "pulse"
        elif "emphasis" in mood_json["reasoning"].lower():
            mood_json["light_behavior"]["effect"] = "pulse"
        elif "calm" in mood_json["reasoning"].lower() or "relaxed" in mood_json["reasoning"].lower():
            mood_json["light_behavior"]["effect"] = "steady"
        else:
            # Default to steady if no specific emotional intent is detected
            mood_json["light_behavior"]["effect"] = "steady"
...
def stop_performance():
    ...
    instruction_box.config(state=tk.NORMAL)
    set_light_state(0, 0, 0, False)  # Turn off the light
    log_event("PERFORMANCE", "Performance stopped")

start_button = tk.Button(root, text="Start Performance", command=start_performance)
start_button.pack()

stop_button = tk.Button(root, text="Stop Performance", command=stop_performance)
stop_button.pack()
stop_button.config(state=tk.DISABLED)
