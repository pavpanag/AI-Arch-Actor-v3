...
BRIDGE_IP = "192.168.1.94"
#"192.168.1.106"  for Streetly router
...
# Global variables for light mapping
light_mapping = {}

def fetch_light_ids():
    """Fetch light IDs dynamically and map them to their names."""
    global light_mapping
    url = f"http://{BRIDGE_IP}/api/{USER_API}/lights"
    response = requests.get(url)
    if response.status_code == 200:
        lights = response.json()
        light_mapping = {light_info['name']: light_id for light_id, light_info in lights.items()}
        print(f"Light mapping: {light_mapping}")
    else:
        print(f"Failed to fetch lights: {response.status_code}")
        light_mapping = {}

def set_light_state(light_name, brightness, hue, saturation):
    """Set the state of a light by its name."""
    light_id = light_mapping.get(light_name)
    if not light_id:
        print(f"Light '{light_name}' not found in mapping.")
        return
    ...
    if response.status_code == 200:
        print(f"Light '{light_name}' set to brightness {brightness}, hue {hue}, saturation {saturation}")
    else:
        print(f"Failed to set light '{light_name}'")

def create_color_field_and_brightness_slider(parent, light_name, row, column):
    brightness = tk.IntVar(value=254)  # Default brightness

    def update_light(event=None):
        bri = brightness.get()
        hue = hue_vars[light_name]
        sat = sat_vars[light_name]
        set_light_state(light_name, bri, hue, sat)
    ...
    def on_canvas_click(event):
        ...
        hue_vars[light_name] = int(hue / 360 * 65535)
        sat_vars[light_name] = int(saturation * 254)
        update_light()

    label = tk.Label(parent, text=f"Light {light_name}")
    ...
    image_path = os.path.join(script_dir, "app.png")
    img = Image.open(image_path).resize((256, 256), Image.Resampling.LANCZOS).convert("RGB")
    image = img
...
def record_memory():
    ...
    memory[current_memory] = [(brightness_vars[light_name].get(), hue_vars[light_name], sat_vars[light_name]) for light_name in light_mapping.keys()]
...
def select_memory(memory_id):
    ...
    for light_name, (bri, hue, sat) in zip(light_mapping.keys(), memory[memory_id]):
        brightness_vars[light_name].set(bri)
        hue_vars[light_name] = hue
        sat_vars[light_name] = sat
        set_light_state(light_name, bri, hue, sat)
...
# Fetch light IDs dynamically
fetch_light_ids()

brightness_vars = {name: None for name in light_mapping.keys()}
hue_vars = {name: 0 for name in light_mapping.keys()}
sat_vars = {name: 0 for name in light_mapping.keys()}

for idx, light_name in enumerate(light_mapping.keys()):
    row, column = divmod(idx, 4)
    brightness_vars[light_name] = create_color_field_and_brightness_slider(lights_frame, light_name, row * 3, column)
