...
#BRIDGE_IP = "192.168.1.106" #for home router
#BRIDGE_IP = "192.168.1.2" #for mesimeri router
BRIDGE_IP = "192.168.1.40"  # grey studio router
USER_API = "0fLeSuFEFbk1UV2ehHFZKAyOBDL7dlSbE2szNqwR"

# Memory for storing light states
memory = {i: [(0, 0, 0) for _ in range(4)] for i in range(1, 21)}  # Default to (0, 0, 0) for lights 1-4
current_memory = 1
...
def record_memory():
    ...
    memory[current_memory] = [(brightness_vars[light_id].get(), hue_vars[light_id], sat_vars[light_id]) for light_id in range(1, 5)]
...
# Initialize Tkinter
root = tk.Tk()
root.title("Philips Hue Light Control (Lights 1-4)")
...
brightness_vars = {i: None for i in range(1, 5)}
hue_vars = {i: 0 for i in range(1, 5)}
sat_vars = {i: 0 for i in range(1, 5)}
