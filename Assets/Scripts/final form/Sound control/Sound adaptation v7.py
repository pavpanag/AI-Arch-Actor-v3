...
OSC_PORT = 4444        # Same port as your light script
SAMPLE_RATE = 44100
NUM_MEMORIES = 8
...
NOISE_THRESHOLD = 68   # Adjust this for the noise gate
...
def record_audio(memory_index, device_index):
    ...
    global is_recording
    is_recording = True
    filename = f"sound_{memory_index+1}.wav"
...
def handle_osc(address, *args):
    """
    We handle all incoming messages (e.g. "/gesture" "sound 1"), 
    ...
    if address == "/gesture":
        if len(args) > 0:
            ...
            if len(parts) == 2:
                ...
                if cmd == "sound":
                    ...
                    # You might implement a "memory" concept if you want, 
                    # or handle it differently. For now, just show a message.
                    print(f"[OSC] memory {num_str} received (not implemented for sound).")
                else:
        ...
        # If you also send messages like /sound 3 (args=(3,)) 
        # you could parse that here:
        try:
            mem_index = int(args[0]) - 1
            if 0 <= mem_index < NUM_MEMORIES:
                play_audio(mem_index)
        except (ValueError, IndexError) as e:
            print(f"[OSC] Invalid /sound argument: {args} - {e}")
    else:
        print(f"[OSC] Ignoring message on {address}")
