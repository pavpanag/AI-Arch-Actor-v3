...
def process_positions(positions):
    ...
    # Normalize positions to percentages and round to integers
    latest_positions = [
    ...
    # Send OSC messages for positions
    for pos in latest_positions:
        osc_client.send_message("/position", [int(pos[0]), int(pos[1])])  # Ensure positions are sent as arrays

    # Calculate and send distances between all pairs of positions
    for i in range(len(latest_positions)):
        for j in range(i + 1, len(latest_positions)):
            distance = calculate_distance(latest_positions[i], latest_positions[j])
            osc_client.send_message("/distance", [[int(distance)]])  # Wrap distance in an array
